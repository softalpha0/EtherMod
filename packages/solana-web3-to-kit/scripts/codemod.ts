...
const RPC_METHODS = [
  "getBalance", "getAccountInfo", "getLatestBlockhash", "getSlot",
  "getTransaction", "getSignatureStatuses", "getTokenAccountsByOwner",
  "getProgramAccounts", "getRecentBlockhash", "confirmTransaction", "requestAirdrop",
];
...
const codemod: Codemod<TSX> = (root) => {
  ...
  // 1. Package rename
  rootNode.findAll('"@solana/web3.js"').forEach((node) => {
  ...
  rootNode.findAll("Keypair.fromSecretKey($BYTES)").forEach((node) => {
  ...
  rootNode.findAll("Keypair.fromSeed($SEED)").forEach((node) => {
  ...
  rootNode.findAll("sendRawTransaction").forEach((node) => {
    edits.push(node.replace("sendTransaction"));
  });

  // 11. RPC method calls → append .send()
  for (const method of RPC_METHODS) {
    rootNode.findAll(`$OBJ.${method}($$$ARGS)`).forEach((node) => {
